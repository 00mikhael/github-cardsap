...
function AddUsersForm({setUsers}) {
  ...
  return (
    <div className="w-full h-40 bg-gray-100 border border-gray-200 p-4 flex flex-col shadow-inner">
      <div className="self-center sm:w-2/3 lg:w-1/2 min-w-max z-10">
        <div className="h-6">
        ...
        <form onSubmit={handleSubmit} className="flex w-full overflow-hidden rounded-md">
          <div className="flex-1">
            <input onChange={handleSearch} autoComplete="off" spellCheck="false" name="userName" value={username} ref={inputRef} style={{caretColor: `${isError ? `red` : `green`}`}} className={`p-4 w-full text-sm font-bold text-gray-700 h-12 border-l border-t border-b border-transparent hover:border-gray-300 focus-within:bg-gray-50 bg-gray-200 hover:bg-gray-300 focus:outline-none shadow-inner rounded-l-md`} type="text" placeholder="Search Github Users" required/>
          ...
          <button className="h-12 w-32 md:w-40 px-4 text-gray-700 text-xs uppercase bg-gray-200 hover:bg-gray-300 border border-gray-300 grid grid-cols-1 grid-rows-1 justify-center items-center gap-4 outline-none focus:outline-none rounded-r-md " type="submit">
  ...
  return (
    <span className="mt-4 p-4 h-auto flex flex-wrap-reverse gap-12 justify-start bg-gray-100">
      {users.length ? users.map(user =>
          ...
          :
          <div style={{minHeight: "60px"}} className="w-full text-center text-gray-600">
            <div className="w-80 mx-auto">
              {intro.id && <div className="w-full shadow-inner p-4 bg-gray-200 typewriter">
